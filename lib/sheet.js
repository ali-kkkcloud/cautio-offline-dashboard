...
//  3. Offline duration    -> only keep rows offline for MORE than
//                            OFFLINE_HOURS (default 48h). Priority:
//                            a) "Offline Since (hrs)" if it's a real number
//                            b) else "Last Online" parsed as a date
//                            c) else the row is reported as "indeterminate"
//                               (not guessed, not silently counted)
...
export function computeDashboard(table) {
  ...
  const offlineRows = [];
  const indeterminateRows = []; // rows we genuinely cannot evaluate — reported, not guessed
  ...
  for (const row of rows) {
    ...
    // Priority:
    //   1) "Offline Since (hrs)" has a real number -> use it directly.
    //   2) Else "Last Online" parses as a date -> compute hours from it.
    //   3) Else -> indeterminate, not counted either way.
    ...
    if (hoursOffline === null || Number.isNaN(hoursOffline)) {
      indeterminateRows.push({ client, vehicle, city });
      continue;
    }

    if (hoursOffline <= OFFLINE_HOURS) continue; // not offline long enough
  ...
  for (const r of offlineRows) {
    if (!clientMap.has(r.client)) {
      clientMap.set(r.client, { client: r.client, count: 0, cities: new Set(), vehicles: [] });
    }
    const entry = clientMap.get(r.client);
    entry.count += 1;
    if (r.city) entry.cities.add(r.city);
    entry.vehicles.push({ vehicle: r.vehicle, city: r.city, hoursOffline: r.hoursOffline });
  }
  ...
  const clients = Array.from(clientMap.values())
    .map((c) => ({
      ...
      vehicles: c.vehicles.sort((a, b) => b.hoursOffline - a.hoursOffline),
    }))
  ...
  return {
    ...
    stats: {
      ...
      criticalClients,
      citiesAffected,
      indeterminateCount: indeterminateRows.length,
    },
    clients,
    cities,
    indeterminate: indeterminateRows.slice(0, 500),
  };
}
