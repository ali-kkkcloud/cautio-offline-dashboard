...
export default function TrendChart() {
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-border bg-panel p-3.5">
        <h3 className="mb-2.5 text-sm font-semibold text-slate-200">Offline Trend (Last 7 Days)</h3>
        <div className="h-[175px] animate-pulse rounded-lg bg-panel2" />
      </div>
    );
  }
  ...
  if (!trend?.configured) {
    return (
      <div className="rounded-xl border border-border bg-panel p-3.5">
        <h3 className="mb-2.5 text-sm font-semibold text-slate-200">Offline Trend (Last 7 Days)</h3>
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-2 py-2 text-[11px] leading-snug text-amber-200">
  ...
  if (known.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-panel p-3.5">
        <h3 className="mb-2.5 text-sm font-semibold text-slate-200">Offline Trend (Last 7 Days)</h3>
        <p className="text-[11px] text-slate-500">No snapshots recorded yet today.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border bg-panel p-3.5">
      ...
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full">
        ...
        {gridValues.map((v, i) => (
          <text key={i} x={PAD_L - 6} y={yFor(v) + 3} textAnchor="end" fontSize="9" fill="#64748b">
        ...
        {known.map((p, i) => (
          <g key={p.label + i}>
            <circle cx={xFor(i)} cy={yFor(p.count)} r="3" fill="#f87171" stroke="#0a1120" strokeWidth="1" />
            <text x={xFor(i)} y={yFor(p.count) - 7} textAnchor="middle" fontSize="9" fontWeight="600" fill="#e2e8f0">
              {p.count.toLocaleString("en-IN")}
            </text>
            <text x={xFor(i)} y={HEIGHT - 4} textAnchor="middle" fontSize="8.5" fill="#64748b">
              {p.label}
            </text>
          </g>
        ))}
